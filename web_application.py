...
if aggregation == "Sales per Day":
    ...
    # Group by Country
    sales_data = df_filtered.groupby("Country", as_index=False)["TotalPrice"].sum().sort_values(by="TotalPrice", ascending=False)
    ...
    sales_data = df_filtered.groupby("Description", as_index=False)["TotalPrice"].sum().sort_values(by="TotalPrice", ascending=False)
